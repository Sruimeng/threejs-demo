...
import { generateAoSampleKernelInitializer, generateMagicSquareNoise, AOShader, AoBlendShader, AODepthToNormalShader, AODepthShader } from './ao-shader';
...
export class AOPass extends Pass {
    ...
    _renderNormalFromDepth: boolean;
    _visibilityCache: Map<any, any>;
    intensity: number;
    pdRings: number;
    pdRadiusExponent: number;
    pdSamples: number;
    aoNoiseTextureMagicSquare: any;
    aoNoiseTextureRandom: DataTexture;
    pdNoiseTexture: DataTexture;
    aoaoRenderTarget: any;
    pdRenderTarget: any;
    aoMaterial: ShaderMaterial;
    normalMaterial: MeshNormalMaterial;
    depthToNormalRenderMaterial: ShaderMaterial;
	...
	constructor( scene: Scene, camera: PerspectiveCamera, width: number, height: number, parameters: any, aoParameters: any, pdParameters: any ) {
	...
	setGBuffer( depthTexture: DepthTexture, normalTexture: Texture ) {
	...
	updateAoMaterial( parameters: any ) {
		...
		if ( parameters.distanceFallOff !== undefined ) {
			...
			this.aoMaterial.uniforms.distanceFallOff.value = parameters.distanceFallOff;
	...
	updatePdMaterial( parameters: any ) {
	...
	override render( renderer: WebGLRenderer, writeBuffer: WebGLRenderTarget, readBuffer: WebGLRenderTarget /*, deltaTime, maskActive */ ) {
		...
		switch ( this.output ) {
			...
			case AOPass.OUTPUT.Diffuse:
				...
				this.renderPass( renderer, this.copyMaterial, this.renderToScreen ? null : writeBuffer , 0xffffff, 1.0 );
